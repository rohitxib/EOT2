...
class AuditDetailVC: UIViewController, MFMailComposeViewControllerDelegate, OptionViewDelegate  {
    
    @IBOutlet weak var auditDes_hieght: NSLayoutConstraint!
    @IBOutlet weak var img_hieght: NSLayoutConstraint!
    @IBOutlet weak var deslbl_Hieght: NSLayoutConstraint!
    @IBOutlet weak var des_hieght: NSLayoutConstraint!
    @IBOutlet weak var txtView: UITextView!
    @IBOutlet weak var btnAddCustomField: UIButton!
    @IBOutlet weak var custTitleLbl: UILabel!
    @IBOutlet weak var customFieldLbl: UILabel!
   ...
   // var jobArray = [AuditListData]()
    var jobArray = [AuditOfflineList]()
    var arrOfCustomForm = [TestDetails]()
    
    var arrTypeStatus : [taskStatusTypeAudit] = [.New, .OnHold, .Completed]
...
//    var auditDetail : AuditListData?
    ...
    var job : UserJobList?
    var statusId : taskStatusTypeAudit?
    var sltDate : String? = ""
    
   
    var Start = LanguageKey.start
    var OnHold = LanguageKey.new_on_hold
    var Completed = LanguageKey.completed
    var arrOfQuestions = [getQuestionsBy]()
    var StrTitless = String()
    var arrOfShowDatas : GetDetailAudit!
    ...
    func setLocalization() -> Void {
     ...
     //   viewBtn.setTitle(LanguageKey.view , for: .normal)
       ...
       // acceptBtn.setTitle(LanguageKey.accept , for: .normal)
        ...
        self.btnAddCustomField.setTitle(LanguageKey.add , for: .normal)
        ...
        custTitleLbl.text = LanguageKey.title_cutom_field
...
//        for fw in fiewldWorkers {
//                    if nameArrr == ""{
//                        nameArrr.append(("\(String(describing: fw.fnm!)) \(String(describing: fw.lnm!))"))
//                        nameArr.append(nameArrr)
//                    }else{
//                        nameArrr = "\(("\(String(describing: fw.fnm!)) \(String(describing: fw.lnm!))"))"
//                     //   nameArr.append(nameArrr)
//                    }
               // }
...
//        }
        ...
        if let statusId = auditDetail?.status{
            let status = taskStatusAudit(taskType: taskStatusTypeAudit(rawValue: Int(statusId == "0" ? "1" : (statusId))!)!)
            ...
            DispatchQueue.main.async {
                ...
                self.isFirst = true
                self.ChangeButtonsAccordingToStatusType(status: taskStatusTypeAudit(rawValue: Int(statusId == "0" ? "1" : (statusId))!)!)
                self.isFirst = false
            }
...
//        }
        ...
        DispatchQueue.global(qos: .background).async {
                   ...
                   DispatchQueue.global(qos: .background).async {
                       let htmlString = self.auditDetail?.des
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if getDefaultSettings()?.isHideContact == "1" && auditDetail?.status == String(taskStatusTypeAudit.Start.rawValue){
    ...
    @IBAction func acceptBtn(_ sender: UIButton) {
        ...
        let status : taskStatusTypeAudit = taskStatusTypeAudit(rawValue: sender.tag)!
    ...
    func ChangeButtonsAccordingToStatusType(status : taskStatusTypeAudit){
        
        if !isFirst{
            
            if status == taskStatusTypeAudit.InProgress{
                for job in jobArray {
                    if job.status == String(taskStatusTypeAudit.InProgress.rawValue){
                        job.status = String(taskStatusTypeAudit.OnHold.rawValue)
        ...
        let statusDetail = taskStatusAudit(taskType: status)
        ...
        case .New:
           ...
           acceptBtn.tag = taskStatusTypeAudit.InProgress.rawValue
           self.acceptBtn.setTitle(LanguageKey.start, for: .normal)
           //self.declineBtn.setTitle("Startfgdgdfg", for: .normal)
           self.accept_WrBtn.constant = -375
        ...
        case .InProgress:
            ...
            acceptBtn.tag = taskStatusTypeAudit.OnHold.rawValue
            self.acceptBtn.setTitle(LanguageKey.new_on_hold, for: .normal)
            
            declineBtn.tag = taskStatusTypeAudit.Completed.rawValue
    ...
    @IBAction func AuditCustomFld(_ sender: Any) {
        
        //let storyboard = UIStoryboard(name: "MainJob", bundle: nil)
               let feedVC = storyboard?.instantiateViewController(withIdentifier: "AuditCoustomField") as! AuditCoustomField
               // feedVC.fff = arrOfShowDatas
               feedVC.sltNaviCount = 0
               feedVC.clickedEvent = nil
               feedVC.isCameFrmStatusBtnClk = true
               feedVC.objOfUserJobList = self.auditDetail!
...
//    }
    ...
    func optionView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
       // return arrTypeStatus.count
        var LangArray  = ["\(self.Start)","\(self.OnHold)","\(self.Completed)"]
    ...
    func optionView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        LocationManager.shared.isHaveLocation()
        let status : taskStatusTypeAudit = arrTypeStatus[indexPath.row]
    ...
    func getFormDetail(){
        ...
        serverCommunicator(url: Service.getFormDetail, param: param.toDictionary) { (response, success) in
            if(success){
                ...
                if let decodedData = try? decoder.decode(FormDetailAudit.self, from: response as! Data) {
                    ...
                    if decodedData.success == true{
                        ...
                        if decodedData.data.count > 0 {
                            DispatchQueue.main.async {
                                ...
                                self.arrOfQuestions = decodedData.data as! [getQuestionsBy]
                                // var ascs = self.arrOfQuestions
                         
                                var strTitless = ""
                                var jiId = self.arrOfQuestions
                                var strTitle = ""
                                for jtid in jiId {
                                    let des = jtid.des
                                    let daretype = jtid.type
                                    ///
                                    strTitless = strTitless + des!
                                    
                                    let answere = jtid.ans
                                    ...
                                    if answere.count > 0 {
                                        for ans in answere {
                                            ...
                                            if ans1 != nil {
                                                self.btnAddCustomField.setTitle(LanguageKey.edit , for: .normal)
                                            }
                                ...
                                self.customFieldLbl.text = strTitless
