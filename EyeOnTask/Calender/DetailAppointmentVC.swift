...
//  EyeOnTask
//
//  Created by Dharmendra Gour on 06/07/20.
...
class DetailAppointmentVC: UIViewController , MFMailComposeViewControllerDelegate ,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    
    @IBOutlet weak var complateStatustBtn: UIButton!
    ...
    @IBOutlet weak var height_Selectall: NSLayoutConstraint!
     ...
     var imgArr = [ImageModel_DetailAppointmentVC]()
    ...
    var isQuoteNoShowOnAppointment = true
    override func viewDidLoad() {
        super.viewDidLoad()
        let currentTimeZone = getCurrentTimeZone()
        print(currentTimeZone)
        if appintDetainsDicCommon?.status == "9"{
            self.complateStatustBtn.isUserInteractionEnabled = false
...
//
       // floatinActionButton()
        ...
        if commanIdBool == true {
            let appoint =  commanId//appintDetainsDicCommon?.commonId?.components(separatedBy: "-")
            if appoint.count > 0 {
                       let appId = commanId//appoint![0]
                       if appId == "Appointment"{
                        ...
                        self.imgEmtAttch.isHidden = false
                           ...
                           isSync = false
                            let image = appintDetainsDicCommon?.attachments
                           if image != nil && image != "" {
            ...
            if commanIdBool == true {
                ...
                if commanIdBool == true {
                    ...
                    let appoint = appintDetainsDicCommon?.commonId?.components(separatedBy: "-")
                           if appoint!.count > 0 {
                               let appId = appoint![0]
                               if appId == "Appointment"{
        ...
        isQuoteNoShowOnAppointment = compPermissionVisible(permission: compPermission.isQuoteNoShowOnAppointment)
        
        
        if isQuoteNoShowOnAppointment == false {
            quotHieght.constant = 0
            lblQuotation.isHidden = true
            lblQuotTitle.isHidden = true
        }else{
            let quot = appintDetainsDicCommon?.quotLabel
            if quot == nil{
                quotHieght.constant = 0
                lblQuotation.isHidden = true
                lblQuotTitle.isHidden = true
...
//                self.refreshControl.endRefreshing()
    ...
    addItem.titleLabel.text = LanguageKey.title_add_job
    addItem.imageView.image = UIImage(named: "Layer 15-1")
    addItem.buttonColor = UIColor(red: 55.0/255.0, green: 132.0/255.0, blue: 141.0/255.0, alpha: 1.0)

    let quotButton = actionButton.addItem()
    quotButton.titleLabel.text = LanguageKey.create_quotation
    quotButton.imageView.image = UIImage(named: "icons8-quote")
    quotButton.buttonColor = UIColor(red: 55.0/255.0, green: 132.0/255.0, blue: 141.0/255.0, alpha: 1.0)

   

        quotButton.action = { [self] item in
    // Do something

    ActivityLog(module:Modules.invoice.rawValue , message:"Edit Appointment")
    let storyboard = UIStoryboard(name: "Quote", bundle: nil)
        if !isHaveNetowork() {
                              DispatchQueue.main.async {
            ...
            if self.commanIdBool == true {
                let appoint = self.commanId//self.appintDetainsDicCommon?.commonId?.components(separatedBy: "-")
                       if appoint.count > 0 {
                        let appId = self.commanId//appoint![0]
                           if appId == "Appointment"{
                           
                               ShowError(message: LanguageKey.appointment_not_sync, controller: windowController)
                            return
                           }else{
                            let vc = storyboard.instantiateViewController(withIdentifier: "AddQuoteVC") as! AddQuoteVC
                                         vc.aadquote = self.appintDetainsDicCommon
                                         vc.boolQuat = true
                                         self.navigationItem.backBarButtonItem = UIBarButtonItem(title:nil, style: .plain, target: nil, action: nil)
                ...
                let appoint = self.appintDetainsDicCommon?.commonId?.components(separatedBy: "-")
                       if appoint!.count > 0 {
                           let appId = appoint![0]
                           if appId == "Appointment"{
        ...
        addItem.action = { item in
                             ...
                             vc.parmnew  = true
                             vc.aadAppId = self.appIdData
                             vc.appoinmnrA = self.appintDetainsDicCommon
          //  print(self.appintDetainsDicCommon)
                             vc.userName = self.appintDetainsDicCommon?.nm != nil ? self.appintDetainsDicCommon?.nm : "Unknown"
...
//     }
    ...
    // setup method
        ...
        func setupMethod() -> Void {
           ...
           //Hide background view
                   ...
                   lblName.text = appintDetainsDicCommon?.nm != nil ? appintDetainsDicCommon?.nm : "Unknown"
                   
                   // Append Address detail==============================
                   let adr = appintDetainsDicCommon?.adr != "" ? appintDetainsDicCommon?.adr : ""
                   
                   let city = appintDetainsDicCommon?.city != "" ? appintDetainsDicCommon?.city : ""
                  
                 let des = appintDetainsDicCommon?.des != "" ? appintDetainsDicCommon?.des : ""
                   lblDesDetail.text = des
            let linkQout = appintDetainsDicCommon?.quotLabel != "" ? appintDetainsDicCommon?.quotLabel : ""
            linkLbl.text = linkQout
            
            

        if appintDetainsDicCommon?.mob1 != "" {
            lblmobileNo.setTitle(appintDetainsDicCommon?.mob1, for: .normal)
        }else{
        ...
        if appintDetainsDicCommon?.mob2 != "" {
            lblPhoneNo.setTitle(appintDetainsDicCommon?.mob2, for: .normal)
        }else{
            ...
            lblPhoneNo.setTitleColor(UIColor.lightGray, for: .normal)
                   ...
                   if appintDetainsDicCommon?.email != "" {
                             btnEmailAction.setTitle(appintDetainsDicCommon?.email, for: .normal)
                          }else{
                             btnEmailAction.isUserInteractionEnabled = false
                              ...
                              btnEmailAction.setTitleColor(UIColor.lightGray, for: .normal)
                                 ...
                                 var ctry = appintDetainsDicCommon?.ctry != "" ? appintDetainsDicCommon?.ctry : ""
                                 var stt = appintDetainsDicCommon?.state != "" ? appintDetainsDicCommon?.state : ""
                   ...
                   if let mark = appintDetainsDicCommon?.landmark {
          ...
          //  time End Date Show
            ...
            if appintDetainsDicCommon?.schdlStart != "" {

                if let enableCustomForm = getDefaultSettings()?.is24hrFormatEnable{ //This is round off digit for invoice
                    if enableCustomForm == "0"{
                       let startDate = (appintDetainsDicCommon?.schdlStart != nil) ? convertTimeStampToString(timestamp: (appintDetainsDicCommon?.schdlStart!)!, dateFormate: DateFormate.ddMMMyyyy_hh_mma) : ""
                                      ...
                                      let endDate = (appintDetainsDicCommon?.schdlFinish != nil) ? convertTimeStampToString(timestamp: appintDetainsDicCommon!.schdlFinish!, dateFormate: DateFormate.ddMMMyyyy_hh_mma) : ""
                       ...
                       let startDate = (appintDetainsDicCommon?.schdlStart != nil) ? convertTimeStampToString(timestamp: (appintDetainsDicCommon?.schdlStart!)!, dateFormate: DateFormate.dd_MM_yyyy_HH_mm) : ""
                                      ...
                                      let endDate = (appintDetainsDicCommon?.schdlFinish != nil) ? convertTimeStampToString(timestamp: appintDetainsDicCommon!.schdlFinish!, dateFormate: DateFormate.dd_MM_yyyy_HH_mm) : ""
            ...
            if commanIdBool == true{
                let searchQuery = "appId = '\(commanId )'"
                ...
                let searchQuery = "appId = '\(appintDetainsDicCommon?.commonId! ?? "")'"
                                               let isExist = DatabaseClass.shared.fetchDataFromDatabse(entityName: "AppointmentList", query: searchQuery) as! [AppointmentList]
                                               if isExist.count > 0 {
                                                   let dta = isExist[0]
            ...
            DispatchQueue.global(qos: .background).async {
                ...
                DispatchQueue.global(qos: .background).async {
                    let htmlString = self.appintDetainsDicCommon?.des
    ...
    @IBAction func linkQuotationBtn(_ sender: Any) {
        ...
        //appintDetainsDicCommon?.quotLabel
        let isExist = self.arrOFUserDataQuot.contains(where: {$0.quotId == appintDetainsDicCommon?.quotId})
        
        let quote = self.arrOFUserDataQuot.filter({$0.quotId == appintDetainsDicCommon?.quotId})
    ...
    @IBAction func statusActionBtn(_ sender: UIButton) {
           ...
           lblStatus.text = LanguageKey.completed
           lblStatus.textColor = UIColor(red: 176/256, green: 180/256, blue: 189/256, alpha: 1.0)
         if appintDetainsDicCommon?.status == "12" {
             UpadateAppointment()
         }
    ...
    @IBAction func btnMap_Action(_ sender: Any) {
        ...
        if (appintDetainsDicCommon?.lat == nil && appintDetainsDicCommon?.lng == nil) ||  (appintDetainsDicCommon?.lat == "" && appintDetainsDicCommon?.lng == "")  {
            ...
            if appintDetainsDicCommon?.lat != "" && appintDetainsDicCommon?.lng != ""  {
                if appintDetainsDicCommon?.lat == "0" {
                    if (lblAddress.text != nil && lblAddress.text != "")  {
                        if UIApplication.shared.canOpenURL(URL(string: "comgooglemaps://")!){
                            openGoogleMap(latitude: "", longitude: "",locationAddress: self.lblAddress.text!)
                        } else  {
                    ...
                    if  UIApplication.shared.canOpenURL(URL(string: "comgooglemaps://")!){
                                       openGoogleMap(latitude: appintDetainsDicCommon!.lat!, longitude: appintDetainsDicCommon!.lng!)
                                   } else  {
                                     //  print(appintDetainsDicCommon!.lat!)
                                       openAppleMap(lat: appintDetainsDicCommon!.lat!, long: appintDetainsDicCommon!.lng!)
                                   }
                }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        ...
        if segue.destination is EditAppointmentVC
        {
            ...
            if commanIdBool == true {
                vc?.commanIdBool = true
                vc?.commanIdEdit = commanId
                vc?.editArrayData = appintDetainsDicCommon
            }else {
                vc?.editArrayData = appintDetainsDicCommon
            }
        ...
        if (status == .denied || status == .restricted) {
            ...
            PHPhotoLibrary.requestAuthorization { (authStatus) in
                ...
                if info[UIImagePickerController.InfoKey(rawValue: UIImagePickerController.InfoKey(rawValue: self.convertFromUIImagePickerControllerInfoKey(UIImagePickerController.InfoKey.originalImage)).rawValue)] != nil{
                            let imgStruct = ImageModel_DetailAppointmentVC(img: (info[.originalImage] as? UIImage)?.resizeImage_DetailAppointmentVC(targetSize: CGSize(width: 1000.0, height: 10000.0)))
                            DispatchQueue.main.async {
  ...
  func uploadDocuments(imgName : String, imageDes : String, completion : @escaping (()->(Void))) -> Void {
    ...
    if commanIdBool == true {
        param.appId = commanId
    }else{
        param.appId = appintDetainsDicCommon?.commonId
    }
    ...
    @IBAction func btnCall_Action(_ sender: Any) {
        if ((appintDetainsDicCommon?.mob1 == "") && (appintDetainsDicCommon?.mob2 == "")) {
    ...
    @IBAction func btnMail_Action(_ sender: Any) {
        ...
        if appintDetainsDicCommon?.email == nil || appintDetainsDicCommon?.email == "" {
                ...
                if let decodedData = try? decoder.decode(AddAppointmentRes.self, from: response as! Data) {
                    ...
                    if decodedData.success == true {
                        ...
                        DispatchQueue.main.async{
                         ...
                         //   var hight:Int = 1193
                                self.collectionView.isHidden = false
                           //  self.topView_H.constant = 1000
                            if (decodedData.data?.attachments!.count)! == 0 {
                                
                                 self.topView_H.constant = 700
                                self.textView_hieght.constant = 100
                               
                                
                            }
                            
                            if (decodedData.data?.attachments!.count)! == 1 {
                                        self.textView_hieght.constant = 200
                                 self.topView_H.constant = 1000
                                                           }
                                                       
                              if (decodedData.data?.attachments!.count)! >= 2 {
                                        self.textView_hieght.constant = 400
                                 self.topView_H.constant = 1200
                                }
...
extension DetailAppointmentVC: UICollectionViewDelegate,UICollectionViewDataSource {
...
extension DetailAppointmentVC: UIDocumentPickerDelegate {
    ...
    //                                self.arrOfFilterDict.removeAll()
        ...
        func UpadateAppointment(){
            ...
            let param = Params()
            param.cltId = appintDetainsDicCommon?.cltId
            param.conId = appintDetainsDicCommon?.conId
            param.siteId = appintDetainsDicCommon?.siteId
            ...
            param.nm = appintDetainsDicCommon?.nm
            param.des = appintDetainsDicCommon?.des
            param.email = appintDetainsDicCommon?.email
            param.mob1 = appintDetainsDicCommon?.mob1
            param.mob2 = appintDetainsDicCommon?.mob2
            param.adr = appintDetainsDicCommon?.adr
            param.cnm = appintDetainsDicCommon?.cnm
            param.city = appintDetainsDicCommon?.city
            param.zip = appintDetainsDicCommon?.des
            ...
            param.schdlStart = appintDetainsDicCommon?.schdlStart
            param.schdlFinish = appintDetainsDicCommon?.schdlFinish
...
struct ImageModel_DetailAppointmentVC {
    var img: UIImage?
   
}

extension UIImage {
    func resizeImage_DetailAppointmentVC(targetSize: CGSize) -> UIImage {
        let size = self.size
